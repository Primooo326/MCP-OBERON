...
export function registerWhaTool(server: McpServer, apiClient: AxiosInstance) {
    ...
    server.tool(
        ...
        {
            mensaje: z.string().describe("El texto del mensaje que se enviará por WhatsApp."),
        ...
        async ({ mensaje, to, replyMessageId }) => {
            ...
            try {
                ...
                const apiKeyHeader = apiClient.defaults.headers.common['x-api-key'] || apiClient.defaults.headers.common['Authorization'];
                if (!apiKeyHeader) {
